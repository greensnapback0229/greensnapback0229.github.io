---
layout: post
title: (MagicOfConch) FCM 알림 배치 100만건 최적화기 - 1
...
private List<UserStreakItem> fetchNextPage() {
    ...
    try {
        ...
        List<UserStreakItem> items = new ArrayList<>(results.size());
        ...
        for (Object[] row : results) {
            ...
            items.add(new UserStreakItem(userInfo, firstReviewDate, reviewCount));
        }

        UserStreakItem lastItem = items.get(items.size() - 1);
        lastId = lastItem.userInfo().getId();
...
또 해결책을 찾다가 Driving Query Pattern이라는 것을 찾을 수 있었습니다. 
...
`JpaCursorItemReader` 의 한계와 Keyset Paging Reader로 OOM을 넘긴 과정까지 정리했습니다.

이 다음부터는 DB 읽기보다 더 큰 병목이었던 **외부 I/O, RabbitMQ 분리, Worker concurrency, Partitioning** 에 대해서 최적화한 과정을 작성합니다.

- [이어서 보기]({% post_url 2026-04-10-(MagicOfConch)-FCM-알림-배치-100만건-최적화기-2 %})
