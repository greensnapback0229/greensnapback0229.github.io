...
처리 도중 Writer에서 일부만 INSERT 된 상태로 트랜잭션이 롤백 되었다고 가정해보겠습니다. chunk commit이 일어나지 않았기 때문에 `update()`도 호출되지 않고, `ExecutionContext`의 `nextOffset`은 chunk 3의 시작 위치(100) 그대로 남아있게 됩니다.

재시작하면 Reader는 nextOffset부터 정확히 이어서 읽지만, Writer 입장에서는 **직전 시도에서 이미 INSERT 했던 일부 행을 다시 받게 됩니다.** Reader가 이어서 읽는 것과는 별개로 생기는 문제입니다.

리밸런싱 레포트의 경우 `goal_id` + `base_date`가 사실상 멱등키 입니다. 같은 goal에 대해서 같은 날짜의 레포트가 두 개 생길 수는 없는 비즈니스 상황이기 때문에 해당 조합을 활용해서 멱등성을 확보할 수 있었습니다. 

처음에는 Writer 안에서 `SELECT 후 없으면 INSERT` 하는 방법을 떠올렸는데 이건 동시성 환경에서 race condition이 생길 수 있고 DB를 2번 찔러야 했기 때문에 선택하지 않았습니다. 결국 DB 레벨에서 처리하는 게 가장 깔끔하다는 결론에 도달했습니다.
...
이렇게 되면 **재시도된 행이 들어와도 unique index에 걸려서 UPDATE로 처리**됩니다. Writer 코드는 그냥 `INSERT`만 호출하면 되고, 멱등성은 DB가 보장해줍니다.
