...
전체 흐름을 그려보면 다음과 같습니다. 정상 실행되는 동안에는 chunk commit과 같은 트랜잭션으로 `nextOffset`을 `ExecutionContext`에 저장해두고, 도중에 step이 실패하더라도 재시작 시 `open()`에서 그 위치를 복원해서 이어서 처리하는 구조입니다.

![rebalance_retry_sequence]({{ page.image_dir }}/rebalance_retry_sequence.svg)
...
기존 시스템을 중단시키고 retry했을때 

```text
chunk N 처리 중:
  1. Reader가 50건 read
  2. Processor가 50건 변환
  3. Writer가 30건 INSERT 까지 진행한 시점에 트랜잭션 롤백 → 실패
  4. chunk commit이 일어나지 않았으므로 update() 호출 안됨
  5. ExecutionContext의 nextOffset은 chunk N의 시작 위치 그대로

재시작:
  6. Reader가 chunk N의 시작 위치부터 다시 50건 read
  7. Writer가 같은 30건을 또 INSERT 시도
```

재시작 자체는 정상적인 흐름이지만, Writer 입장에서는 **이미 한 번 INSERT 했던 데이터를 다시 받게됩니다.** Reader가 이어서 읽는 것과 별개로 발생하는 문제입니다.
